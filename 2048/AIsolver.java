/*package com.cs323.assign1.agent;
...
public class AIsolver {
    ...
    public static Direction findBestMove(Board theBoard, int depth) throws CloneNotSupportedException {
        ...
        if(ConsoleGame.algo== true){//alphabeta
        ...
        System.out.println("  Time taken to findBestMove= "+(endTime-startTime)+" ms.");
        ConsoleGame.sumScore+=(endTime-startTime);
        return (Direction)result.get("Direction");
    }
    // Method that finds the best next move ( for AVG case call)
    //@Overloads
    public static Direction findBestMove(Board theBoard, int depth,int flag) throws CloneNotSupportedException {
        long startTime=System.currentTimeMillis();
        Map<String, Object> result ;
        if(ConsoleGame.algo== true){//alphabeta
            result = alphabeta(theBoard, depth, Integer.MIN_VALUE, Integer.MAX_VALUE, Player.USER);
